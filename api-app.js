(() => {
  ...
  app.innerHTML = `
    ...
    <main class="en-main">
          ...
          <section class="en-card">
            ...
            <div class="en-form-grid">
              <div>
                ...
                <select id="apiGradeLevel" class="en-select"><option value="PreK">PreK</option><option value="K">K</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option></select>
              </div>
              <div>
      ...
      <section id="apiBillingView" class="hidden">
        ...
        <p class="en-lede">Use demo selection for local testing, or launch Stripe checkout for paid pilots.</p>
        <div class="plan-grid">
          ...
          <article class="plan selected"><h3>Family</h3><div class="plan-price" id="apiFamilyPrice">$8</div><button class="en-button apiPlan" data-plan="family">Choose Family demo</button><button class="en-outline apiCheckout" data-plan="family" style="margin-top:8px">Start paid checkout</button></article>
          <article class="plan"><h3>Classroom</h3><div class="plan-price">$18</div><button class="en-outline apiPlan" data-plan="classroom">Choose Classroom demo</button><button class="en-outline apiCheckout" data-plan="classroom" style="margin-top:8px">Start paid checkout</button></article>
        </div>
  ...
  function renderStoryList() {
    ...
    container.innerHTML = stories.map(s => {
      ...
      return `<div class="story-row" style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #eee"><div><div style="font-weight:700">${esc(s.title)}</div><div style="font-size:12px;color:#666">${esc(detail)}${s.created_by ? ` • generated by ${esc(friendlySource(s.created_by))}` : ''}</div>${objectiveLabel ? `<div style="font-size:11px;color:#888;margin-top:2px">Objective: ${esc(objectiveLabel)}</div>` : ''}</div><div><button class="open-story en-button" data-id="${s.id}" style="min-width:86px">Open</button></div></div>`;
    }).join('');
  ...
  async function openServerStory(story) {
    ...
    inner.innerHTML = `<div class="book-modal-head"><h2 class="book-modal-title">${esc(story.title)}</h2><button id="closeApiStory" class="en-outline">Close</button></div><div class="book-modal-meta">${esc(modalMeta)}</div>${modalObjective ? `<div class="book-modal-meta" style="margin-top:4px">Objective: ${esc(modalObjective)}</div>` : ''}<img class="book-modal-hero" src="${themeBannerMap[story.theme] || themeBannerMap.Moonlight}" alt="Story illustration"><div id="apiStoryPages"></div><div class="book-modal-actions"><button id="apiCompleteStory" class="en-button">Mark completed</button></div><div id="apiUpgradeCta" class="book-modal-upgrade"></div><h3 class="book-modal-subtitle">Talk about it</h3><div id="apiStoryQuestions" class="book-modal-list"></div><h3 class="book-modal-subtitle">Word garden</h3><div id="apiStoryWords" class="book-modal-list"></div>`;
    ...
    renderPage();
    inner.querySelector('#apiStoryQuestions').innerHTML = (story?.content?.questions || []).map(q => `<p>• ${esc(q)}</p>`).join('') || '<p class="book-modal-empty">No questions provided.</p>';
  ...
  async function refresh() {
    const [meData, learnerData, storyData, subscriptionData, scoreData, offerData, reminderData] = await Promise.all([
      api('/api/me'),
      api('/api/learners'),
      api('/api/stories'),
    ...
    $('#apiStats').innerHTML = `<div class="en-stat"><strong>${learners.length}</strong><span>learners</span></div><div class="en-stat"><strong>${stories.length}</strong><span>stories saved</span></div><div class="en-stat"><strong>${stories.filter(s => s.completed_at).length}</strong><span>completed</span></div>`;
