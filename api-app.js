(() => {
  ...
  function renderStoryList() {
    ...
    container.innerHTML = stories.map(s => {
      ...
      return `<div class="story-row" style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #eee"><div><div style="font-weight:700">${esc(s.title)}</div><div style="font-size:12px;color:#666">${esc(detail)}${s.created_by ? ` • generated by ${esc(friendlySource(s.created_by))}` : ''}</div><div style="font-size:10px;color:#888;margin-top:2px">Story ID: ${esc(s.id)}</div>${objectiveLabel ? `<div style="font-size:11px;color:#888;margin-top:2px">Objective: ${esc(objectiveLabel)}</div>` : ''}</div><div><button class="open-story en-button" data-id="${s.id}" style="min-width:86px">Open</button></div></div>`;
    }).join('');
  ...
  async function openServerStory(story) {
    ...
    assessment.onsubmit = async (event) => {
      event.preventDefault();
      try {
        const responses = [...assessment.querySelectorAll('.apiAssessmentAnswer')].map(input => input.value);
