...
app.delete('/api/stories/:storyId', requireAuth, requireParentConfirmation, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(`UPDATE stories s SET deleted_at = COALESCE(s.deleted_at, NOW()) FROM learners l WHERE s.id = $1 AND l.account_id = $2 AND s.learner_id = l.id AND s.deleted_at IS NULL${scope} RETURNING s.id`, params);
    if (!result.rowCount) return res.status(404).json({ error: 'Story not found.' });
...
app.get('/api/classroom/progress-summary.pdf', requireAuth, async (req, res, next) => {
  try {
    ...
    const learnersRes = await pool.query(`
      ...
      FROM learners l
      LEFT JOIN stories s ON s.learner_id = l.id AND s.deleted_at IS NULL
