...
app.post('/api/stories/:storyId/assessment', requireAuth, async (req, res, next) => {
  try {
    ...
    await pool.query('INSERT INTO reading_assessments (id, story_id, learner_id, responses, score, mastered, review_status, reviewed_at, confidence, standards_evidence) VALUES ($1, $2, $3, $4, $5, $6, $7, CASE WHEN $8 THEN NOW() ELSE NULL END, $9, $10)', [randomUUID(), req.params.storyId, storyRes.rows[0].learner_id, JSON.stringify(responses), score, autoScored && score >= 80, autoScored ? 'auto_scored' : 'pending', autoScored, autoScored ? 0.95 : 0.5, evidence]);
