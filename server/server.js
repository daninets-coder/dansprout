...
}).refine(value => !value.confirmPassword || value.password === value.confirmPassword, { path: ['confirmPassword'], message: 'Passwords must match.' });
...
async function requireChildStoryScope(req, res, next) {
  if (req.auth?.childMode !== true) return next();
  try {
    const result = await pool.query('SELECT s.id FROM stories s JOIN learners l ON l.id = s.learner_id WHERE s.id = $1 AND l.account_id = $2 AND s.learner_id = $3', [req.params.storyId, req.auth.sub, req.auth.learnerId]);
...
app.post('/api/auth/child-login', loginLimiter, async (req, res, next) => {
  try {
    const parsed = z.object({ username: z.string().trim().min(3).max(32), password: z.string().min(1).max(128) }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: 'Enter your child username and password.' });
    const result = await pool.query('SELECT l.id, l.account_id, l.first_name, l.age_band, l.child_password_hash, a.email_verified_at FROM learners l JOIN accounts a ON a.id = l.account_id WHERE LOWER(l.child_username) = LOWER($1)', [parsed.data.username]);
    const learner = result.rows[0];
    if (!learner?.child_password_hash || !(await bcrypt.compare(parsed.data.password, learner.child_password_hash))) return res.status(401).json({ error: 'Child username or password is incorrect.' });
    if (!learner.email_verified_at) return res.status(403).json({ error: 'The adult account must verify its email before a child can sign in.' });
    return res.json({ token: childModeTokenFor(learner.account_id, learner.id), account: { childMode: true, learner: { id: learner.id, first_name: learner.first_name, age_band: learner.age_band } } });
  } catch (error) {
    return next(error);
  }
});
...
app.patch('/api/learners/:learnerId/child-login', requireAuth, requireParentConfirmation, async (req, res, next) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ error: 'Use a child username with 3 to 32 letters or numbers and a password of at least 6 characters.' });
    const passwordHash = await bcrypt.hash(parsed.data.password, 12);
    const result = await pool.query('UPDATE learners SET child_username = $1, child_password_hash = $2, updated_at = NOW() WHERE id = $3 AND account_id = $4 RETURNING id, first_name, child_username', [parsed.data.username.toLowerCase(), passwordHash, req.params.learnerId, req.auth.sub]);
    if (!result.rowCount) return res.status(404).json({ error: 'Learner not found.' });
    return res.json({ learner: result.rows[0] });
  } catch (error) {
    if (error.code === '23505') return res.status(409).json({ error: 'That child username is already in use. Choose another one.' });
    return next(error);
  }
});
...
app.get('/api/progress', requireAuth, async (req, res, next) => { try { const scope = req.auth.childMode === true ? ' AND l.id = $2' : ''; const params = req.auth.childMode === true ? [req.auth.sub, req.auth.learnerId] : [req.auth.sub]; const { rows } = await pool.query(`SELECT l.id, l.first_name, l.age_band, COUNT(s.id)::int AS stories_created, COUNT(s.completed_at)::int AS stories_completed, COALESCE(json_agg(DISTINCT s.learning_goal) FILTER (WHERE s.learning_goal IS NOT NULL), '[]') AS learning_goals, (SELECT COUNT(*)::int FROM reading_assessments ra WHERE ra.learner_id = l.id AND ra.mastered = TRUE AND ra.review_status IN ('auto_scored', 'reviewed', 'corrected')) AS mastered_assessments, COALESCE((SELECT ROUND(AVG(ra.score))::int FROM reading_assessments ra WHERE ra.learner_id = l.id AND ra.review_status IN ('auto_scored', 'reviewed', 'corrected')), 0) AS average_assessment_score, COALESCE((SELECT COUNT(*)::int FROM reading_assessments ra WHERE ra.learner_id = l.id), 0) AS assessments_completed, (SELECT json_build_object('storyId', latest.id, 'title', latest.title, 'learningGoal', latest.learning_goal, 'objective', latest.content->'meta'->>'curriculumObjective', 'vocabulary', COALESCE(latest.content->'words', '[]'::jsonb), 'completedAt', latest.completed_at, 'createdAt', latest.created_at, 'assessment', (SELECT json_build_object('score', ra.score, 'answered', COALESCE((ra.standards_evidence->>'answered')::int, jsonb_array_length(ra.responses)), 'questionCount', COALESCE((ra.standards_evidence->>'questionCount')::int, jsonb_array_length(latest.content->'questions'))) FROM reading_assessments ra WHERE ra.story_id = latest.id ORDER BY ra.created_at DESC LIMIT 1)) FROM stories latest WHERE latest.learner_id = l.id ORDER BY latest.created_at DESC LIMIT 1) AS last_activity FROM learners l LEFT JOIN stories s ON s.learner_id = l.id WHERE l.account_id = $1${scope} GROUP BY l.id ORDER BY l.created_at`, params); return res.json({ learners: rows }); } catch (error) { return next(error); } });
