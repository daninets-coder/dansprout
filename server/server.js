...
app.post('/api/subscription/cancel', requireAuth, async (req, res, next) => { try { const bodySchema = z.object({ reason: z.string().trim().max(120).optional() }); const parsed = bodySchema.safeParse(req.body || {}); if (!parsed.success) return res.status(400).json({ error: 'Please check the cancellation request.' }); const subscriptionRes = await pool.query('SELECT id, plan, status, provider_subscription_id FROM subscriptions WHERE account_id = $1 AND status IN ($2, $3) ORDER BY updated_at DESC LIMIT 1', [req.auth.sub, 'active', 'demo']); const subscription = subscriptionRes.rows[0]; if (!subscription || subscription.plan === 'explorer') return res.json({ subscription: { plan: 'explorer', status: 'canceled' }, message: 'There is no paid subscription to cancel.' }); let effectiveAt = null; if (stripe && subscription.provider_subscription_id) { const remote = await stripe.subscriptions.update(subscription.provider_subscription_id, { cancel_at_period_end: true }); effectiveAt = remote.current_period_end ? new Date(remote.current_period_end * 1000).toISOString() : null; } const result = await pool.query('UPDATE subscriptions SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING plan, status', ['canceled', subscription.id]); return res.json({ subscription: result.rows[0], effectiveAt, message: effectiveAt ? `Your ${subscription.plan} plan will remain available until ${new Date(effectiveAt).toLocaleDateString()}. No future renewal will be charged.` : `Your ${subscription.plan} plan was canceled. Your learner data remains saved.` }); } catch (error) { return next(error); } });
